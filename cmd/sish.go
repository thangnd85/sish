...
func init() {
	...
	rootCmd.PersistentFlags().StringP("redirect-root-location", "r", "https://www.hass.bid", "The location to redirect requests to the root domain\nto instead of responding with a 404")
	...
	rootCmd.PersistentFlags().StringP("domain", "d", "hass.bid", "The root domain for HTTP(S) multiplexing that will be appended to subdomains")
